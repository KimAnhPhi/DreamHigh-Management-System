...
**HTTP API / NestJS (and similar):** When the stack uses per-domain modules, the structure you document MUST include `dto/` (or stack-equivalent folder) per domain and MUST align with Step 5: **Request + Response** boundary artifacts colocated there — not only input DTOs.
...
Each domain module under `src/modules/<domain>/` should normally include `dto/` for **Request + Response** contracts (see Implementation Patterns, Step 5).
