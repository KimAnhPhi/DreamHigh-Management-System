...
- **Security & compliance profile (khuyến nghị dự án production):** `{planning_artifacts}/architecture-security-profile.md` hoặc file `*security-profile*.md` trong `{planning_artifacts}` — copy từ `./data/architecture-security-profile.template.md` của skill nếu chưa có
