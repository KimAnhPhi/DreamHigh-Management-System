...
- **Nếu đã load `architecture-security-profile.md` (hoặc `*security-profile*.md`):** mở đầu category bằng tóm tắt **ASVS level / data classes / attack surface** từ profile; mọi gợi ý (auth, API security, encryption, logging, secrets) phải **bám profile** và ghi rõ mục ASVS hoặc NFR nào được đáp ứng. Nếu profile yêu cầu control mà starter chưa có → đưa vào quyết định hoặc **TODO kiến trúc** có lý do.
- **Nếu không có profile:** vẫn suy luận từ PRD (PII, thanh toán, API public…); **nhắc user** tạo `architecture-security-profile.md` từ template để gợi ý bảo mật nhất quán cho dự án thật.
...
- API boundary typing: **Request + Response** contracts at the HTTP edge (DTO/schema — detailed patterns in Step 5)
