...
- **If `architecture-security-profile.md` was loaded:** Does the architecture explicitly address the stated **ASVS target level** (map major areas: auth/session, access control, validation, crypto, logging, configuration, API security)? List **gaps** or **deferred controls** with rationale; do not claim full ASVS compliance unless decisions + patterns substantiate it.
